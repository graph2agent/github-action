...
[[ -n "$token" ]] || fail "GRAPH2AGENT_READ_TOKEN is required"
...
# Git reads this authorization header only from the child process environment.
# No credential helper, remote URL, global config, or repository config is
# modified. Do not enable shell tracing in this script.

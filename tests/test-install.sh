...
captured=$(PATH="${fake_path}:$PATH" \
  ...
  GRAPH2AGENT_VERSION=v0.1.0 \
  RUNNER_TEMP="$temporary" \
  GITHUB_OUTPUT="$github_output" \
  "${repo_root}/scripts/install.sh" 2>&1)

[[ $captured == *'Installed graph2agent v0.1.0'* ]]
[[ $captured != *"$test_token"* ]]
[[ $(tr '\0' '\n' <"$go_log") == $'install\ngithub.com/graph2agent/graph2agent/cmd/graph2agent@v0.1.0' ]]
[[ $(grep -c '^binary=' "$github_output") -eq 1 ]]
grep -Fx 'version=v0.1.0' "$github_output" >/dev/null
...
if PATH="${fake_path}:$PATH" \
  ...
  GITHUB_OUTPUT="$github_output" \
  "${repo_root}/scripts/install.sh" >/dev/null 2>&1; then
  printf 'install accepted a missing read token\n' >&2
  exit 1
fi
